...
if quay_username == "" or quay_token == "":
    print(
        "Please provide quay user (env QUAY_USER) and quay token (env QUAY_TOKEN) or set the value in a '.env' file "
        "with export command in PWD")
    exit(1)
basic = auth.HTTPBasicAuth(quay_username, quay_token)
authenticated_request = requests_cache.CachedSession(cache_file_location, auth=basic, expire_after=timedelta(days=3))
